...
u8 KEYPAD_GetKey(void)
{
	...
	for (i=0;i<ROWS;i++)
	{
		DIO_WritePin(FIRST_OUTPUT+i,LOW);     //put (zero for first row) to show which pin get (zero in first column)
		for (j=0;j<COLS;j++)
		{
		...
		DIO_WritePin(FIRST_OUTPUT+i,HIGH);   //must return row high to see next row
	}
	return key;
}
